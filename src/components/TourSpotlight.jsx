import { useEffect, useState, useRef } from 'react'
import { useNavigate } from 'react-router-dom'
import { useMascota } from '../hooks/useMascota'
import { TOUR_PASOS, TOTAL_PASOS_TOUR } from '../data/mascotas'

export default function TourSpotlight({ rutaActual }) {
  const { mascota, tourPaso, avanzarTour } = useMascota()
  const [rect, setRect] = useState(null)
  const [esDesktop, setEsDesktop] = useState(window.innerWidth >= 768)
  const navigate = useNavigate()
  const observerRef = useRef(null)

  useEffect(() => {
    const handleResize = () => setEsDesktop(window.innerWidth >= 768)
    window.addEventListener('resize', handleResize)
    return () => window.removeEventListener('resize', handleResize)
  }, [])

  const pasoInfo = TOUR_PASOS[tourPaso]
  const activo = pasoInfo && pasoInfo.ruta === rutaActual && mascota

  useEffect(() => {
    if (!activo) {
      setRect(null)
      return
    }

    let intentos = 0
    let cleanupFn = null

    const elementoVisible = (el) => {
      if (!el) return false
      const r = el.getBoundingClientRect()
      return r.width > 0 && r.height > 0
    }

    const intentarObservar = () => {
      // Busca el elemento visible: primero desktop, luego mobile
      const elDesktop = document.getElementById(pasoInfo.targetId)
      const elMobile  = document.getElementById(pasoInfo.targetId + '-mobile')
      const el = elementoVisible(elDesktop)
        ? elDesktop
        : elementoVisible(elMobile)
          ? elMobile
          : null

      if (!el) {
        intentos++
        if (intentos < 20) setTimeout(intentarObservar, 100)
        return
      }

      const actualizar = () => setRect(el.getBoundingClientRect())
      setTimeout(actualizar, 50)

      const observer = new ResizeObserver(actualizar)
      observer.observe(el)
      observerRef.current = observer

      window.addEventListener('scroll', actualizar, true)
      window.addEventListener('resize', actualizar)

      cleanupFn = () => {
        observer.disconnect()
        window.removeEventListener('scroll', actualizar, true)
        window.removeEventListener('resize', actualizar)
      }
    }

    intentarObservar()

    return () => {
      if (observerRef.current) observerRef.current.disconnect()
      if (cleanupFn) cleanupFn()
    }
  }, [activo, pasoInfo?.targetId, esDesktop])

  if (!activo || !rect || rect.height === 0) return null

  const handleSiguiente = async () => {
    await avanzarTour()
    if (pasoInfo.siguienteRuta !== rutaActual) {
      navigate(pasoInfo.siguienteRuta)
    }
  }

  const padding = 8
  const top    = rect.top    - padding
  const left   = rect.left   - padding
  const width  = rect.width  + padding * 2
  const height = rect.height + padding * 2

  const ALTURA_DIALOGO = esDesktop ? 160 : 180
  const ANCHO_DIALOGO  = esDesktop ? 360 : undefined
  const MARGEN         = 16
  const viewportH      = window.innerHeight
  const viewportW      = window.innerWidth

  const espacioArriba = top
  const espacioAbajo  = viewportH - (top + height)

  let dialogoTop
  if (espacioAbajo >= ALTURA_DIALOGO + MARGEN) {
    dialogoTop = top + height + MARGEN
  } else if (espacioArriba >= ALTURA_DIALOGO + MARGEN) {
    dialogoTop = top - ALTURA_DIALOGO - MARGEN
  } else {
    dialogoTop = Math.max(MARGEN, (viewportH - ALTURA_DIALOGO) / 2)
  }

  let dialogoLeft, dialogoRight, translateX
  if (esDesktop) {
    const centroCuadro = left + width / 2
    const mitadDialogo = ANCHO_DIALOGO / 2
    if (centroCuadro - mitadDialogo < MARGEN) {
      dialogoLeft = MARGEN
      translateX = '0'
    } else if (centroCuadro + mitadDialogo > viewportW - MARGEN) {
      dialogoLeft = 'auto'
      dialogoRight = MARGEN
      translateX = '0'
    } else {
      dialogoLeft = centroCuadro
      translateX = '-50%'
    }
  }

  const esUltimo = tourPaso === TOTAL_PASOS_TOUR

  return (
    <div className="fixed inset-0 z-[100]">
      <div
        className="absolute rounded-2xl transition-all duration-200"
        style={{
          top, left, width, height,
          boxShadow: '0 0 0 9999px rgba(0,0,0,0.6)',
        }}
      />

      {/* MOBILE */}
      {!esDesktop && (
        <div
          className="absolute left-1/2 -translate-x-1/2 w-[calc(100%-32px)] max-w-sm"
          style={{ top: dialogoTop }}
        >
          <div className="bg-white rounded-card shadow-lg p-4 flex gap-3 items-start">
            <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 bg-brand-50">
              <img src={mascota.imagen} alt={mascota.nombre} className="w-full h-full object-cover" />
            </div>
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                <p className="text-sm font-bold text-gray-800">{mascota.nombre}</p>
                <p className="text-[10px] text-gray-400">{tourPaso}/{TOTAL_PASOS_TOUR}</p>
              </div>
              <p className="text-sm text-gray-600">{pasoInfo.texto}</p>
              <button
                onClick={handleSiguiente}
                className="mt-3 bg-brand-600 text-white text-xs font-medium rounded-button px-4 py-2"
              >
                {esUltimo ? '¡Listo! →' : 'Entendido →'}
              </button>
            </div>
          </div>
        </div>
      )}

      {/* DESKTOP */}
      {esDesktop && (
        <div
          className="absolute"
          style={{
            top: dialogoTop,
            left: dialogoLeft !== 'auto' ? dialogoLeft : undefined,
            right: dialogoRight ?? undefined,
            width: ANCHO_DIALOGO,
            transform: `translateX(${translateX})`,
          }}
        >
          <div className="bg-white rounded-card shadow-xl p-5 flex gap-4 items-start">
            <div className="w-14 h-14 rounded-full overflow-hidden shrink-0 bg-brand-50 ring-2 ring-brand-200">
              <img src={mascota.imagen} alt={mascota.nombre} className="w-full h-full object-cover" />
            </div>
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-bold text-gray-800">{mascota.nombre}</p>
                <div className="flex items-center gap-1.5">
                  {Array.from({ length: TOTAL_PASOS_TOUR }).map((_, i) => (
                    <div
                      key={i}
                      className={`h-1.5 w-4 rounded-full transition-all ${
                        i < tourPaso ? 'bg-brand-600' : 'bg-gray-200'
                      }`}
                    />
                  ))}
                </div>
              </div>
              <p className="text-sm text-gray-600 leading-relaxed">{pasoInfo.texto}</p>
              <div className="flex items-center justify-between mt-4">
                <button
                  onClick={async () => {
                    for (let i = tourPaso; i <= TOTAL_PASOS_TOUR; i++) {
                      await avanzarTour()
                    }
                  }}
                  className="text-xs text-gray-400 hover:text-gray-600 transition"
                >
                  Saltar tour
                </button>
                <button
                  onClick={handleSiguiente}
                  className="bg-brand-600 hover:bg-brand-700 text-white text-xs font-medium rounded-button px-5 py-2 transition"
                >
                  {esUltimo ? '¡Listo! →' : 'Entendido →'}
                </button>
              </div>
            </div>
          </div>
          {espacioAbajo >= ALTURA_DIALOGO + MARGEN && (
            <div
              className="absolute -top-2 w-4 h-4 bg-white rotate-45 shadow-sm"
              style={{ left: Math.min(Math.max(left + width / 2 - (dialogoLeft || 0) - 8, 16), ANCHO_DIALOGO - 32) }}
            />
          )}
        </div>
      )}
    </div>
  )
}